package org.gluu.agama.businessforgetusername;
...
import java.util.*;
import org.gluu.agama.businessforgetusername.smtp.*;
import org.gluu.agama.businessforgetusername.smtp.EmailUsernameAr;
import org.gluu.agama.businessforgetusername.smtp.EmailUsernameEn;
import org.gluu.agama.businessforgetusername.smtp.EmailUsernameEs;
import org.gluu.agama.businessforgetusername.smtp.EmailUsernameId;
import org.gluu.agama.businessforgetusername.smtp.EmailUsernamePt;
import org.gluu.agama.businessforgetusername.smtp.EmailUsernameFr;


import org.gluu.agama.businessusernameclass.UsernameResendclass;
...
public class JansForgetUsername extends UsernameResendclass {
    ...
    public Map<String, String> getUserEntityByMail(String email) {
        ...
        // Step 2a: Restrict this flow to business accounts only.
        // Business accounts are marked by a non-empty businessName attribute. Personal
        // accounts (no businessName) are rejected here by returning null — the same surface
        // as user-not-found — so the personal/business distinction is never observable to
        // the caller. Personal users must use the personal forget-username flow instead.
        Object orgVal = user.getAttribute("businessName", true, false);
        if (orgVal == null || orgVal.toString().trim().isEmpty()) {
            logger.warn("Forgot username rejected — not a business account for email: {}", email);
            return null;
        }
